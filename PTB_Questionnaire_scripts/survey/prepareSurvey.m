...
% for likert survey, adding text hint for the two extremes
hints = {"Nem értek egyet", [], [], [], [], [], "Egyetértek"}
    ...
    case 'likert'
        for i = 1:length(questions)
            ...
            for j = 1:ansNum
                ...
                if j==1 || j==7 
                  DrawFormattedText(paperTexture, hints{j}, (j-0.8)*ansW, newy+28, 0);
                  %DrawFormattedText(paperTexture, hints{j}, (j*ansW/2)/(length(hints{j})/2), newy+28, 0);  
                end    
            end
        end
end
