function bgMain_v2(pairNo, player, labName, confType, confNo)
...
%
% Optional inputs:
% confType  - Numeric value, one of 1:5. Defines the tpye of initial config
%           to use, defaults to 1.
% confNo    - Numeric value, one of 1:20. Defines the config number to use
%           from given config type. Defaults to 1.
%
...
if nargin < 5 || isempty(confNo)
    confNo = 1;
end
if nargin < 4 || isempty(confType)
    confType = 1;
end
if nargin < 3
    error('Input args "pairNo", "player" and "labName" are required!');
end
...
videoSaveFile = ["pair", num2str(pairNo), labName, "_times.mat"];
...
tmp = load([baseDir, "easyConfigs.mat"]);
...
% % shuffle
%imgFiles = imgFiles(randperm(numel(imgFiles)));
...
imgLoc = [0.1, 0.05;...  % defines img center coordinates (x, y) in scale 0-1, where (0, 0) is the top left corner
          0.1, 0.25;...  
          0.1, 0.45;...
          0.1, 0.65;...
          0.9, 0.05;...
          ...
          0.9, 0.45;...
          0.9, 0.65];
imgTargetSize = [150, 150];  % size for images - images should be already resized into this size
...
%             0.63, 0.40;...
%             0.83, 0.40];
priceLoc = [imgLoc(:, 1)+0.03, imgLoc(:, 2)+0.1];
priceRectSize = [60, 60];
...
%             0.57, 0.40;...
%             0.77, 0.40];
shelvesNoLoc = [imgLoc(:, 1)-0.03, imgLoc(:, 2)+0.1];
shelvesNoRectSize = [60, 60];

% about counters - depends on "counterTypesMax"
counterLoc = [0.10, 0.85;...  % positions similarly to "imgLoc"
          0.20, 0.85;...
          0.30, 0.85;...
          0.40, 0.85];
...
totalWealthLoc = [0.5, 0.80];
totalWealthRectSize = [60, 60];
playerOfferLoc = [0.4, 80];
playerOfferRectSize = [60, 60];
otherOfferLoc = [0.6, 80];
...
try
    ...
    instrText = ["Egy 'Alku játék' következik." char(10), char(10),... 
                ...
                "és hogy közben összegyűjtsd a számodra célként kijelölt tárgyakat.", char(10), char(10),...
                "Egy bal klikkel jelezheted, ha készen állsz."];
    ...
    while GetSecs < firstFlip + timeout
        ...
        if ~bargainUpdate
            ...
            if changeFlag  || otherChange || tex > 0
                ...
                playerOfferValue = counterState.*tokenPrices;
    ...
    save(videoSaveFile, "sharedStartTime", "videoCaptureStartTime",...
