function bgMain_v11(pairNo, player, labName, confType, confNo, normImgNo, strangeImgNo)
...
if nargin < 7 || isempty(strangeImgNo)
    strangeImgNo = 0;
end
if nargin < 6 || isempty(normImgNo)
    normImgNo = 8;
end
...
if (normImgNo+strangeImgNo) > 8
    error('The sum of input args "normImgNo" and "strangeImgNo" must be less than or equal to 8!');
end
...
tmp = load(confF);
conf = tmp.easyConfigs(confNo);

% extract prices, token distribution and must-haves
tokenPrices = conf.prices(1:(normImgNo+strangeImgNo), player);
tokenAmounts = conf.tokens(1:(normImgNo+strangeImgNo), player);
mustHaves = conf.mustHaves(1:(normImgNo+strangeImgNo), player);
...
rand("state", normImgNo+strangeImgNo);
...
timeout = 610;  % max wait time to quit
exitKey = 'Escape';
